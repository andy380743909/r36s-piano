...
-- added for love2d game maker virtual joystick
-- https://www.love2d.org/wiki/GamepadButton
-- 
-- a
...
function love.gamepadpressed(joystick, button)
   ...
   elseif button == "leftstick" then
      ...
      triggerReport = "L-Stick LEFT pressed"
      lstkState[2] = "dn"
      if joystick:isGamepadDown("back") then  -- SELECT + Lstk-LEFT detected
         scene[999].input() -- change input key-map to 999's
         scene.current = 999 -- change to exitscreen scene
      scene.previous = 100
      end
...
function K.draw()
   ...
   if love.keyboard.isScancodeDown("escape") or (virtualJoystick and virtualJoystick:isGamepadDown("back")) then
    	helpText = help[100]
	else
		helpText = ""
    end
